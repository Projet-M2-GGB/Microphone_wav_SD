/**
  ...
  * @brief   Description of the MP3 encoder and decoder application
  ******************************************************************************
  *
   * @attention
  *
...
This application is made to demonstrate MP3 encoder and decoder integration,
it can encode and decode audio data in MP3 format or WAV format.
A first menu enables to select between recorder or player interfaces.
...
++++++ Recorder ++++++
It can encode in MP3 format the audio stream from stereo microphones or
in WAV format by un-commenting the defined WAV_ENC_CHOSEN constant in 
header file audio_conf.h.

In recorder mode, the MP3 module encodes input stereo data stream from
2 microphones at 16Khz sampling rate and records audio file on USB key in 16b
with MP3 compression format set to @128kbps.

A double MEMS microphone is connected to the Inter-IC Sound (I2S) peripheral. 
The I2S is configured in master receiver mode. In this mode, the I2S peripheral 
provides the clock to the MEMS microphones through CLK_IN and acquires the data
Audio samples) from the MEMS microphone through PDM_OUT.
...
The microphones PDM input bit data stream (at 1024kbps) is received in PDM double buffer
of 256 samples in 16-bits (ie 128 stereo samples), handled by DMA. Each half of 128 samples
in 16-bits (64L/64R) corresponds to 1ms stereo frame (@64Khz in 16-bits) and is written
alternatively to perform continuous PDM audio data recording.
Afterward each half buffer is converted in PCM format (@16kHz in 16-bits) before encoding in MP3 format 
and writing to USB audio file.

- The microphones use audio BSP in DMA mode to record data (16 bits).
- At DMA half transfer ISR, The PDM data is converted to PCM 
and written to PCM buffer. 
- At DMA transfer complete ISR, The PDM data is converted to PCM 
and written to PCM buffer.
at each ISR, when half PCM buffer (576 stereo samples in 16-bits) is filled 
an indication is sent to application to indicate that the frame data can be MP3 encoded.

- When limit file size is reached, audio recording is stopped.

A touch screen interface is used to change volume, pause or stop recording and to quit
recorder by pressing BACK button. 
...
@par Directory contents 
  ...
  - Audio_MP3/Src/recorder.c           Audio recorder in MP3 (or WAV)
  - Audio_MP3/Src/stm32f4xx_it.c       Interrupt handlers
  - Audio_MP3/Src/system_stm32f4xx.c   STM32F4xx system source file
  ...
  - Audio_MP3/Inc/recorder.h           Audio recorder header file    
  - Audio_MP3/Inc/stm32f4xx_hal_conf.h HAL configuration file
  - Audio_MP3/Inc/stm32f4xx_it.h       Interrupt handlers header file
