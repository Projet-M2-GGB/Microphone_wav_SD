...
#define TOUCH_VOL_XOFFSET      (int)(4*(480.0/272))
...
#define TOUCH_BACK_XOFFSET      (int)(8*(480.0/272))
#define MSG_BACK_YLINE          10

#define PLAYER_CLEAR_XWIDTH       (int)(250*(800.0/480))
...
static __IO uint32_t uwVolumeRef = 70; /*80*/
...
static AUDIO_ErrorTypeDef PlayerGetFileInfo(uint16_t file_idx, Audio_InfoTypeDef *info)
{
  ...
  BSP_LCD_SetTextColor(LCD_COLOR_BLACK);
  BSP_LCD_FillRect(0, LINE(4), PLAYER_CLEAR_XWIDTH+CHAR(10), LINE(1));
  BSP_LCD_SetTextColor(LCD_COLOR_WHITE);
  ...
  BSP_LCD_SetTextColor(LCD_COLOR_BLACK);
  BSP_LCD_FillRect(0, LINE(6), PLAYER_CLEAR_XWIDTH+CHAR(10) , LINE(1));
  BSP_LCD_SetTextColor(LCD_COLOR_WHITE);
...
static void PlayerRecorderDisplayButtons(void)
{
  ...
  BSP_LCD_DrawRect(TOUCH_BACK_XMIN, TOUCH_BACK_YMIN, /* BACK menu */
...
static void AcquireTouchButtons(void)
{
  ...
  else
  {
    ...
    if(TS_State.touchDetected == 1)
    {
      ...
      {
        /* Close audio instance before leaving player mode*/
        FilePos = 0;
        PLAYER_Stop();
        PLAYER_Close();
