...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define  EXTI_Line0                       EXTI0_IRQn        /*!< STM32F4XX Interrupt Number Definition on line0*/
...
/* Private functions ---------------------------------------------------------*/
extern void SWI_EXTI0_Callback(void); //weak
...
void EXTI0_IRQHandler(void)
{
  ...
  //HAL_NVIC_ClearPendingIRQ(EXTI_Line0); //needed ????

  EXTI->PR = EXTI_PR_PR0;

  //if it is SWI on EXTI0 do...
  if ( is_swi )
  {
    //callback call
    SWI_EXTI0_Callback();
...
{
  HAL_DMA_IRQHandler(haudio_out_sai.hdmatx);  //SAI2 (for audio output DAC)
}
